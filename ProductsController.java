...
public class ProductsController implements ActionListener, MouseListener, KeyListener {
    ...
    public ProductsController(Products products, ProductsDao productDao, SystemView views) {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == views.btn_register_product) {
            if (views.txt_product_code.getText().equals("")
                ...
                if (productDao.registerProductQuery(products)) {
                    cleanTable();
                    cleanFields();
                    listAllProducts();
            ...
            if (views.txt_product_code.getText().equals("")
                ...
                if(productDao.updateProductQuery(products)){
                    cleanTable(); 
                    cleanFields(); 
                    listAllProducts(); 
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == views.products_table) {
            int row = views.products_table.rowAtPoint(e.getPoint()); //metodo para obtener las coordenadas de la fila donde la persona dio click
            views.txt_product_id.setText(views.products_table.getValueAt(row, 0).toString());
            products = productDao.searchProduct(Integer.parseInt(views.txt_product_id.getText()));
            ...
            views.cmb_product_category.setSelectedItem(new DynamicCombobox(products.getCategory_id(), products.getCategory_name()));
            //Deshabilitar
            views.btn_register_product.setEnabled(false);
